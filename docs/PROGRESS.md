...
- [ ] **Phase 2 — Frontend plumbing:** widen `LevelData.id` to `string`, `encodeRows()` helper, `VisitViewMode` + `"editor"`, API fns in `lib/api.ts`, `useIslandStages` hook. Gate: `npx tsc --noEmit`.
